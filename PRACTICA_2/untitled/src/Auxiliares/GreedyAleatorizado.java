...
public class GreedyAleatorizado {
    ...
    public GreedyAleatorizado() {}

    // Función que realiza el algoritmo Greedy Aleatorio y devuelve un Individuo
    public Individuo RealizarGreedy(int k, long seed, LectorTSP lector) {
        ...
        // Convertir la lista de índices a un int[] y crear el objeto Individuo
        int[] vectorSol = solucion.stream().mapToInt(Integer::intValue).toArray();
        Individuo individuo = new Individuo(vectorSol, lector);

        // Actualizar el mejor coste
